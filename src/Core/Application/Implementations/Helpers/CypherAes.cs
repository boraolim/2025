...
public class CypherAes : ICypherAes
{
    ...
    public CypherAes(Result<string> secretValue)
    {
        if(secretValue.MessageDescription.Length != MainConstantsCore.CFG_MAX_KEY_NUMBER)
            throw new ArgumentException(MessageConstantsCore.MSG_EXACTLY_32_CHARS);

        keySecret = Encoding.UTF8.GetBytes(secretValue.MessageDescription);
    }
    ...
    public string AESDecryptionGCM(string encryptedText)
    {
        ...
        using var aes = new AesGcm(keySecret, tag.Length);
    ...
    public string AESEncryptionGCM(string plainText)
    {
        ...
        // Encrypt
        using var aes = new AesGcm(keySecret, tagSize);
